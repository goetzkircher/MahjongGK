...
'        Dim before As Integer = gen.Stock.Count
'        gen.CheckAndRefillVorrat()
'        Debug.WriteLine($"Refill-Versuch bei Count={before} -> Count={gen.Stock.Count} (sollte unverändert bleiben, wenn >0)")

'        ' Jetzt Vorrat leeren und Refill explizit erlauben
'        gen.Stock.Clear()
'        gen.CheckAndRefillVorrat(refillStoneSet:=True)
'        Debug.WriteLine($"Leer+Refill (StoneSet): Count={gen.Stock.Count} (sollte wieder gefüllt sein)")
'    End Sub
...
'        Dim startCount As Integer = gen.Stock.Count
'        Debug.WriteLine($"Init Stock.Count = {startCount}")

'        ' In kleinen Portionen nachfüllen lassen (unter Schwelle)
'        gen.Stock.Clear()
'        gen.StockNachschubschwelle = 100 ' klein halten, damit Refill greift
'        gen.StockMaxUBound = 400

'        gen.CheckAndRefillVorrat()
'        Debug.WriteLine($"Nach erstem Refill (StoneStream) Count={gen.Stock.Count}")

'        ' **Invariante:** StoneStream liefert Paare => Gesamtzahl stets gerade
'        If (gen.Stock.Count And 1) <> 0 Then
...
'        gen.CheckAndRefillVorrat()
'        Debug.WriteLine($"Nach erneutem Refill: Count={gen.Stock.Count} (<= MaxUBound={gen.StockMaxUBound})")
'    End Sub
...
'            .StockNoSortAreaEndIndex = 5
'        }

'        Debug.WriteLine($"Vor Shuffle (erste 12): {Dump(gen.Stock, 12)}")
'        gen.ShuffleVorrat()
...
'        Debug.WriteLine($"RemovePaareFromVorrat → Rest={cnt}, StopNachschub={gen.StockStopNachschub}")
'        Debug.WriteLine($"Inhalt (bis 20): {Dump(gen.Stock, 20)}")

'        ' Danach wieder freigeben & auffüllen
'        gen.StockStopNachschub = False
'        Debug.WriteLine($"StopNachschub wieder False → Count={gen.Stock.Count}")
'    End Sub
