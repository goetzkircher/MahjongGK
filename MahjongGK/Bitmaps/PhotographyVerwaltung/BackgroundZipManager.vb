...
Public Class BackgroundZipManager
    ...
    'In den Methoden sind in den Schleifen einige Applikation.DoEvents verteilt.
    'Das Auspacken und prüfen geschieht während des Startvorgangs des Programms
    'und dauert beim allererstem Start am längsten. Das spätere reine Prüfen geht
    'deutlich schneller. 
    'Da der Hintergrund von frmMain in dem Paket drin ist, mußte ich den Splash
    'vorschalten, dessen einzige Aufgabe es ist, die sonst entstehende "Leere"
    'zwischen Programmstart und dem Erscheinen von frmMain zu überprücken.
    'Im Splash ist eine kleine Animation. Sie läuft nur, wenn hier genügend
    'Applikation.DoEvents verteilt sind.
    'Ich habe versucht das mit dem BackgroundManager in einem anderem Thread zu 
    'lösen. Es wurde so aufwendig, daß ich die klassische Lösung bevorzugt habe.
    ...
    Public Sub InitializeOnStartup()
        Dim seedZipPath As String = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                                                 "Bitmaps", "Photography", "Hintergrundgrafiken.zip")
