...
#Region "Öffentliche API"
    ...
    Public Sub Load(fullpath As String, mode As BackgroundImageRenderMode)
        ...
        Else
            ...
            ' 3) Laden über Magick – KEIN Throw, interner Try/Catch in Helper
            If String.IsNullOrEmpty(errMsg) Then
                loaded = LoadOriginalBitmapViaMagick(normalized)
...
#Region "Hilfsfunktionen (Loader, Skalierung, Farben)"
    ...
    Private Shared Function LoadOriginalBitmapViaMagick(path As String) As Bitmap
        Try
            Using coll As New MagickImageCollection(path)
                Dim first As MagickImage = Nothing
                If coll.Count > 0 Then
                    first = CType(coll(0).Clone(), MagickImage)
                End If
                If first Is Nothing Then
                    first = New MagickImage(path)
                End If
                first.AutoOrient()
                first.BackgroundColor = MagickColors.Transparent
                Dim bytes As Byte() = first.ToByteArray(MagickFormat.Png)
                first.Dispose()
    ...
    Private Shared Function CreateErrorBitmap(fullpath As String, Optional message As String = Nothing) As Bitmap
        Dim w As Integer = 256, h As Integer = 64
    ...
    ' FitInside, FitInsideNoUpscale, CoverSize, ScaleHighQuality, ComputeDominantColor, RgbToHsv bleiben unverändert 
    ' (wie in meiner letzten Version).
    ...
    ' Dominante Farbe: grobes 16x16x16-Histogramm, leichte Bevorzugung von gesättigten/hellen Pixeln
