...
Public Class frmGfxCompiler
    ...
    Private ReadOnly _rootOutput As String =
               Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
                     "Visual Studio", "MahjongGK", "MahjongGK", "Eigene Ressourcen")
    ...
    Friend Sub WriteMjGfxModule(packs As IEnumerable(Of GfxPack),
        ...
        sb.AppendLine("        Dim bytes As Byte() = InflateGZipFromBase64(entry.DataGzBase64)")
        sb.AppendLine("        Using ms As New MemoryStream(bytes, writable:=False)")
        sb.AppendLine("            Using src As Image = Image.FromStream(ms, useEmbeddedColorManagement:=True, validateImageData:=True)")
        sb.AppendLine("                Return ResizeImageCrisp(CType(src, Bitmap), w, h)")
        sb.AppendLine("            End Using")
        sb.AppendLine("        End Using")
        sb.AppendLine("    End Function")
        sb.AppendLine()
        ...
        ' Resize (hochwertig)
        sb.AppendLine("    Private Function ResizeImageCrisp(src As Bitmap, w As Integer, h As Integer) As Bitmap")
        sb.AppendLine("        If src Is Nothing Then Throw New ArgumentNullException(NameOf(src))")
        sb.AppendLine("        If w <= 0 OrElse h <= 0 Then Throw New ArgumentOutOfRangeException(""w/h"")")
        sb.AppendLine()
        sb.AppendLine("        Dim dst As New Bitmap(w, h, Drawing.Imaging.PixelFormat.Format32bppArgb)")
        sb.AppendLine("        ' DPI vom Quellbild übernehmen (falls sinnvoll)")
        sb.AppendLine("        Try")
        sb.AppendLine("            dst.SetResolution(src.HorizontalResolution, src.VerticalResolution)")
        sb.AppendLine("        Catch")
        sb.AppendLine("            ' exotic DPI -> Standard belassen")
        sb.AppendLine("        End Try")
        sb.AppendLine()
        sb.AppendLine("        Using g As Graphics = Graphics.FromImage(dst)")
        ...
        sb.AppendLine("            Dim srcRect As New Rectangle(0, 0, src.Width, src.Height)")
        sb.AppendLine()
        ...
        sb.AppendLine("                g.DrawImage(src, destRect, srcRect.X, srcRect.Y, srcRect.Width, srcRect.Height, GraphicsUnit.Pixel, ia)")
        sb.AppendLine("            Else")
        sb.AppendLine("                g.DrawImage(src, destRect, srcRect, GraphicsUnit.Pixel)")
        sb.AppendLine("            End If")
        sb.AppendLine("        End Using")
        sb.AppendLine()
